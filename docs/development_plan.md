...
2. Gjøre admin på mobil mer kompakt med sticky underfaner eller tydeligere separate arbeidsflater for `Styring`, `Del`, `Spillere` og `Kamper`. **Bunnnavigasjon er implementert.**
...
**Gjennomført i Fase 10**

- Mobil arbeidsflatenavigasjon er en kompakt, fast bunnrad uten panel-overlegg.
- Desktop beholder sticky visningsfaner med samme tre arbeidsflater.
- PWA-cache ble bumpet til v58 slik at eksisterende installasjoner henter ny CSS og ny mappestruktur.
- Admin- og spillervisningen har filtre for alle, aktive, neste og ferdige kamper.
...
**Gjennomført i Fase 11**

- Browsermoduler ligger i `app/`, stylesheet i `styles/` og prosjekt-/driftsdokumenter i `docs/`.
- `index.html`, `privacy.html`, `service-worker.js` og testharnessene bruker de nye filstiene.
