...
Arbeidskopien på `codex/padelstar-ui-refresh` inneholder en 0.5 Beta-baseline med implementert kjerneflyt, fase 2–6-formatmoduler, TV Mode, profil/konto, historikk/statistikk, PWA/offline, varsler og konsolidert blå UI-retning. Siste lokale verifikasjon viser `npm test` med 200 tester: 199 bestått og 1 forventet live-Supabase-test hoppet over. `npm run check:syntax` og `git diff --check` passerer.
...
Fase A-kontroll 2026-09-04: de fire siste grensene er testet etter hver flytting. Storage-nøkler, Supabase-config-leser, rene hjelpefunksjoner, DOM-registry, app-meta, theme, bootstrap-events og app-init er nå isolert og lastet fra HTML/service worker, med ferske domenetester og lokal browser-reload. Branchen har fortsatt gjenværende entrypoint-orkestrering i `app/app.js`; fase A står derfor som pågående. Dette er en kontrollert del-leveranse, ikke en ferdigmelding.
