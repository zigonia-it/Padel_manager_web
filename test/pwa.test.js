...
test("service worker claims updates and keeps a navigation fallback", () => {
  assert.match(serviceWorkerSource, /padelstar-v303/);
  ...
  assert.match(indexSource, /app\/match-list\.js\?v=padelstar-match-list-2/);
  assert.match(serviceWorkerSource, /app\/match-list\.js\?v=padelstar-match-list-2/);
  ...
  assert.match(serviceWorkerSource, /padelstar-tournament-state-4/);
  assert.match(serviceWorkerSource, /padelstar-state-bootstrap-1/);
  assert.match(serviceWorkerSource, /padelstar-module-routing-4/);
...
test("remote admin mutations have their own RPC boundary", () => {
  ...
  assert.match(indexSource, /app\/remote-admin-actions\.js\?v=padelstar-remote-admin-actions-2/);
  assert.match(serviceWorkerSource, /app\/remote-admin-actions\.js\?v=padelstar-remote-admin-actions-2/);
...
test("remote player scoring has its own queue boundary", () => {
  ...
  assert.match(indexSource, /app\/remote-player-score\.js\?v=padelstar-remote-player-score-2/);
  assert.match(serviceWorkerSource, /app\/remote-player-score\.js\?v=padelstar-remote-player-score-2/);
...
test("score actions have their own mutation boundary", () => {
  ...
  assert.match(indexSource, /app\/score-actions\.js\?v=padelstar-score-actions-5/);
  assert.match(serviceWorkerSource, /app\/score-actions\.js\?v=padelstar-score-actions-5/);
...
test("global app event wiring has its own boundary", () => {
  ...
  assert.match(indexSource, /app\/app-events\.js\?v=padelstar-app-events-4/);
  assert.match(serviceWorkerSource, /app\/app-events\.js\?v=padelstar-app-events-4/);
...
test("tournament create and join flows have their own event boundary", () => {
  ...
  assert.match(indexSource, /app\/tournament-entry\.js\?v=padelstar-tournament-entry-6/);
  assert.match(serviceWorkerSource, /app\/tournament-entry\.js\?v=padelstar-tournament-entry-6/);
...
test("create tournament wizard has real format/rules choices and its own step boundary", () => {
  ...
  assert.match(indexSource, /app\/create-wizard\.js\?v=padelstar-create-wizard-2/);
  assert.match(serviceWorkerSource, /app\/create-wizard\.js\?v=padelstar-create-wizard-2/);
  ...
  assert.match(appSource, /function createTournament\(\{ name, inviteCode, players, courtCount, format, gamesToWinSet/);
...
test("admin form mutations have their own event boundary", () => {
  ...
  assert.match(indexSource, /app\/admin-form-events\.js\?v=padelstar-admin-form-events-5/);
  assert.match(serviceWorkerSource, /app\/admin-form-events\.js\?v=padelstar-admin-form-events-5/);
...
test("profile session lifecycle has its own storage and RPC boundary", () => {
  ...
  assert.match(indexSource, /app\/bootstrap\/dom-elements\.js\?v=padelstar-dom-elements-11/);
  assert.match(indexSource, /app\/bootstrap\/app-meta\.js\?v=padelstar-app-meta-4/);
  assert.match(indexSource, /app\/ui\/theme\.js\?v=padelstar-theme-1/);
  assert.match(serviceWorkerSource, /app\/bootstrap\/dom-elements\.js\?v=padelstar-dom-elements-11/);
  assert.match(serviceWorkerSource, /app\/bootstrap\/app-meta\.js\?v=padelstar-app-meta-4/);
...
test("match card rendering has its own DOM and action boundary", () => {
  ...
  assert.match(indexSource, /app\/match-card\.js\?v=padelstar-match-card-8/);
  assert.match(serviceWorkerSource, /app\/match-card\.js\?v=padelstar-match-card-8/);
...
test("player next-match rendering has its own workspace boundary", () => {
  ...
  assert.match(indexSource, /app\/player-next-match\.js\?v=padelstar-player-next-match-3/);
  assert.match(serviceWorkerSource, /app\/player-next-match\.js\?v=padelstar-player-next-match-3/);
...
test("rules rendering has its own translation and DOM boundary", () => {
  ...
  assert.match(indexSource, /app\/rules\.js\?v=padelstar-rules-3/);
  assert.match(serviceWorkerSource, /app\/rules\.js\?v=padelstar-rules-3/);
...
test("large score rendering has its own dialog boundary", () => {
  ...
  assert.match(indexSource, /app\/large-score\.js\?v=padelstar-large-score-2/);
  assert.match(serviceWorkerSource, /app\/large-score\.js\?v=padelstar-large-score-2/);
...
test("profile UI rendering has its own form and history boundary", () => {
  ...
  assert.match(indexSource, /app\/profile-ui\.js\?v=padelstar-profile-ui-3/);
  assert.match(serviceWorkerSource, /app\/profile-ui\.js\?v=padelstar-profile-ui-3/);
...
test("player state operations have their own domain boundary", () => {
  ...
  assert.match(indexSource, /app\/player-state\.js\?v=padelstar-player-state-2/);
  assert.match(serviceWorkerSource, /app\/player-state\.js\?v=padelstar-player-state-2/);
...
test("tournament state construction has its own module boundary", () => {
  ...
  assert.match(indexSource, /app\/tournament-state\.js\?v=padelstar-tournament-state-4/);
  assert.match(serviceWorkerSource, /app\/tournament-state\.js\?v=padelstar-tournament-state-4/);
});
...
test("module routing has its own policy boundary", () => {
  ...
  assert.match(serviceWorkerSource, /app\/module-routing\.js\?v=padelstar-module-routing-4/);
});
...
test("central render orchestration has its own module boundary", () => {
  ...
  assert.match(indexSource, /app\/ui\/app-renderer\.js\?v=padelstar-app-renderer-4/);
  assert.match(serviceWorkerSource, /app\/ui\/app-renderer\.js\?v=padelstar-app-renderer-4/);
});
...
test("browser entrypoint and service worker use the same cache-busting versions", () => {
  ...
  assert.match(indexSource, /app\/app\.js\?v=padelstar-session-64/);
  ...
  assert.match(serviceWorkerSource, /app\/app\.js\?v=padelstar-session-64/);
...
test("admin workspace keeps sharing and player management under control", () => {
  assert.equal((indexSource.match(/class="subtab/g) || []).length, 3);
  assert.match(indexSource, /data-admin-panel="control"[\s\S]*data-admin-panel="matches"[\s\S]*data-admin-panel="standings"/);
