-- admin_set_result_impl updated a scored match's `state` field ("finished"/
-- "playing") but never its separate `status` field ("completed"/"active"),
-- leaving the two out of sync (e.g. a finished match stuck showing
-- status: "active"). Harmless today -- admin_advance_round_impl and the
-- client's canCompleteRound/roundProgress both key off `state`, not
-- `status` -- but it's fragile groundwork for a future bug. This adds the
-- matching `status` writes alongside every `state` write this function
-- already makes, with no other behavior change.
