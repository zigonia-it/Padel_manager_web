-- Same class of bug as 20260915040000_admin_set_result_status_sync.sql,
-- found while testing the Cup format end-to-end: admin_advance_round_impl
-- (Round Robin) and admin_advance_cup_impl (Cup) both transition a
-- match's `state` field to "playing" when starting it on a court, but
-- neither ever sets the separate `status` field. For admin_advance_round
-- this leaves status stuck at whatever it was at generation time
-- ("scheduled") -- confirmed against a real finished tournament, where
-- rounds 2/3's matches ended with state: "finished" but status still
-- "scheduled". For admin_advance_cup, newly created bracket matches
-- (regular and third-place) are built via jsonb_build_object without a
-- `status` key at all, so it reads back as SQL null. Harmless today for
-- the same reason as the prior fix -- both admin_advance_*_impl
-- themselves and the client's canCompleteRound/roundProgress key off
-- `state`, not `status` -- but worth closing for the same consistency
-- reason. No other behavior change.
