...
if [[ "${PADELSTAR_SMOKE_VIEWPORT:-desktop}" == "desktop" ]]; then
  run_pw run-code "async (page) => { await page.getByRole('button', { name: 'Opprett' }).last().click(); const widths = await page.evaluate(() => ({ panel: document.querySelector('.setup-panel:not(.hidden)')?.getBoundingClientRect().width, card: document.querySelector('#createTournamentForm')?.getBoundingClientRect().width })); if (!widths.panel || !widths.card || widths.card > widths.panel + 1) throw new Error('setup card exceeds its panel'); await page.getByRole('textbox', { name: 'Turneringsnavn' }).fill('Browser smoke'); await page.getByRole('textbox', { name: 'Spillere, valgfritt' }).fill('Ada\nBo'); await page.getByRole('button', { name: 'Opprett turnering' }).click(); await page.getByRole('button', { name: 'Fortsett uten konto', exact: true }).click(); await page.getByRole('button', { name: 'Start turnering' }).click(); }"
else
