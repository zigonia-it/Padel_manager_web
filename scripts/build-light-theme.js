...
// The stylesheets that make up the app. tv.css is a separate, always-dark page; the theme files themselves are outputs.
const EXCLUDED = new Set(["tv.css", "theme-light.css", "theme-light-manual.css"]);
...
if (require.main === module) {
  ...
  if (process.argv.includes("--check")) {
    ...
    if (current !== generated) { console.error("styles/theme-light.css is out of date: run  node scripts/build-light-theme.js"); process.exit(1); }
    console.log("styles/theme-light.css is up to date");
  } else {
    fs.writeFileSync(OUTPUT, generated);
    console.log(`wrote ${path.relative(root, OUTPUT)} (${generated.split("\n").length} lines, ${sourceFiles().length} source files)`);
  }
}
